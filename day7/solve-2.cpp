...
bool validCombination(int64_t valTillNow, int i, int64_t& testVal, std::vector<int>& nums, int& N) {
    if(i == N-1) {
        return (valTillNow + nums[i] == testVal || valTillNow * nums[i] == testVal || std::stoll(std::to_string(valTillNow) + std::to_string(nums[i])) == testVal);
    }
    return (validCombination(valTillNow + nums[i], i+1, testVal, nums, N) ||
            validCombination(valTillNow * nums[i], i+1, testVal, nums, N) ||
            validCombination(std::stoll(std::to_string(valTillNow) + std::to_string(nums[i])), i+1, testVal, nums, N));
};
